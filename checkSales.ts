...
const buildMessage = (sale: any) => (
  new Discord.MessageEmbed()
	.setColor('#0099ff')
	.setTitle(sale.asset.name + ' listed for '+ethers.utils.formatEther(sale.ending_price)+' '+ethers.constants.EtherSymbol+'')
	...
	.addFields(
		...
		{ name: 'Price', value: `${ethers.utils.formatEther(sale.ending_price)}${ethers.constants.EtherSymbol}`},
		// { name: 'Buyer', value: sale?.winner_account?.address, },
		// { name: 'Seller', value: sale?.seller?.address,  },
	)
  .setImage(sale.asset.image_url)
	.setTimestamp(Date.parse(`${sale?.created_date}Z`))
	// .setFooter('li on OpenSea', 'https://files.readme.io/566c72b-opensea-logomark-full-colored.png')
)
